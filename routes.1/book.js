...
router.get('/list', async function (ctx, next) {
  ...
  result = await service.column("*").select().orderBy(sort, "desc").offset(page * size).limit(size).from('book');
...
router.get('/query/:id', async function (ctx, next) {
  ...
  result = { ...result, ...count, isCollect: !!bag, chapterId: book ? book.chapterId : 0, isRecommend: !!(recommend.count) };
